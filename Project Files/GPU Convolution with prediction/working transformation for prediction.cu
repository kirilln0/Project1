...
//MASK :2X2
int const MASKPADX = convLayerSizeX % 3;
int const MASKPADY = convLayerSizeY % 3;
int const MASKX = 2;
int const MASKY = 2;
...
void Convolution(float* inputMatrix, float* reducedMatrix, float* weights, float* result)
{
	...
	int convXNew = (MaskNum % MASKS_IN_X) * MASKX + Y % MASKX;
	int convYNew = (MaskNum / MASKS_IN_X) * MASKY + (Y % MASK_SIZE) / MASKX;
	int inputX = convXNew * STRIDEX + X % CONV_RECP_SIZEY;
	int inputY = convYNew * STRIDEY + (X % (CONV_RECP_SIZEX * CONV_RECP_SIZEY)) / CONV_RECP_SIZEX;
